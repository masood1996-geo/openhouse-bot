"""
Create or edit a basic OpenHouse configuration
"""
import sys
import re
import os
...
from openhouse.crawler import immobilienscout
...
def welcome():
    ...
    message_dialog(
        title="OpenHouse Configuration Wizard",
        text="Welcome to the OpenHouse Configuration Wizard\n\n"
          "This Wizard will take you through the configuration of your OpenHouse\ninstallation. "
...
def gather_urls(config: YamlConfig) -> List[str]:
    ...
    while first_run or len(urls) == 0 or len(result) > 0:
        ...
        print("OpenHouse scrapes property portals by fetching content from search URLs\n"
...
def configure_mattermost(config: YamlConfig) -> Dict[str, Any]:
    ...
    print("To receive messages over Mattermost, OpenHouse will need the Webhook URL\n"
...
def configure_slack(config: YamlConfig) -> Dict[str, Any]:
    ...
    print("To receive messages over Slack, OpenHouse will need the Webhook URL\n"
...
def configure_captcha(urls: List[str], config: YamlConfig) -> Optional[Dict[str, Any]]:
    ...
    "IMPORTANT NOTICE: Buying captcha credit does not guarantee that OpenHouse will be\n"
