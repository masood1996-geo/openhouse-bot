...
from openhouse.chrome_wrapper import get_chrome_version, CHROME_BINARY_NAMES
from openhouse.exceptions import ChromeNotFound
...
The first returns should all be empty [] so the get_chrome_version function at OpenHouse/chrome_wrapper.py:31
thinks no linux chrome is installed and then checks for windows registry entry at OpenHouse/chrome_wrapper.py:46
Therefore prepending calc_linux_binary_names().
Append the same amount empty returns to the end so OpenHouse/chrome_wrapper.py:31 is forced to check for windows
...
class ChromeWrapperTest(unittest.TestCase):
    ...
    @patch("OpenHouse.chrome_wrapper.get_command_output")
