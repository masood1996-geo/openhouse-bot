...
from openhouse.captcha.captcha_solver import CaptchaSolver
from openhouse.captcha.imagetyperz_solver import ImageTyperzSolver
from openhouse.captcha.twocaptcha_solver import TwoCaptchaSolver
from openhouse.captcha.capmonster_solver import CapmonsterSolver
from openhouse.crawler.kleinanzeigen import Kleinanzeigen
from openhouse.crawler.idealista import Idealista
from openhouse.crawler.immobiliare import Immobiliare
from openhouse.crawler.immobilienscout import Immobilienscout
from openhouse.crawler.immowelt import Immowelt
from openhouse.crawler.wggesucht import WgGesucht
from openhouse.crawler.vrmimmo import VrmImmo
from openhouse.crawler.subito import Subito
from openhouse.crawler.degewo import Degewo
from openhouse.crawler.gewobag import Gewobag
from openhouse.crawler.howoge import Howoge
from openhouse.crawler.gesobau import Gesobau
from openhouse.crawler.wbm import WBM
from openhouse.crawler.stadtundland import StadtUndLand
from openhouse.crawler.llm_crawler import LlmUniversalCrawler
...
class Env:
    ...
    # Captcha setup
    OPENHOUSE_2CAPTCHA_KEY = _read_env("OPENHOUSE_2CAPTCHA_KEY")
    OPENHOUSE_IMAGETYPERZ_TOKEN = _read_env("OPENHOUSE_IMAGETYPERZ_TOKEN")
    OPENHOUSE_CAPMONSTER_KEY = _read_env("OPENHOUSE_CAPMONSTER_KEY")
    OPENHOUSE_HEADLESS_BROWSER = _read_env("OPENHOUSE_HEADLESS_BROWSER")
    OPENHOUSE_IS24_COOKIE = _read_env("OPENHOUSE_IS24_COOKIE")

    # Generic Config
    OPENHOUSE_TARGET_URLS = _read_env("OPENHOUSE_TARGET_URLS")
    OPENHOUSE_DATABASE_LOCATION = _read_env("OPENHOUSE_DATABASE_LOCATION")
    OPENHOUSE_GOOGLE_CLOUD_PROJECT_ID = _read_env(
        "OPENHOUSE_GOOGLE_CLOUD_PROJECT_ID")
    OPENHOUSE_VERBOSE_LOG = _read_env("OPENHOUSE_VERBOSE_LOG")
    OPENHOUSE_LOOP_PERIOD_SECONDS = _read_env(
        "OPENHOUSE_LOOP_PERIOD_SECONDS")
    OPENHOUSE_RANDOM_JITTER_ENABLED = _read_env("OPENHOUSE_RANDOM_JITTER_ENABLED")
    OPENHOUSE_LOOP_PAUSE_FROM = _read_env("OPENHOUSE_LOOP_PAUSE_FROM")
    OPENHOUSE_LOOP_PAUSE_TILL = _read_env("OPENHOUSE_LOOP_PAUSE_TILL")
    OPENHOUSE_MESSAGE_FORMAT = _read_env("OPENHOUSE_MESSAGE_FORMAT")

    # Website setup
    OPENHOUSE_WEBSITE_SESSION_KEY = _read_env(
        "OPENHOUSE_WEBSITE_SESSION_KEY")
    OPENHOUSE_WEBSITE_DOMAIN = _read_env("OPENHOUSE_WEBSITE_DOMAIN")
    OPENHOUSE_WEBSITE_BOT_NAME = _read_env("OPENHOUSE_WEBSITE_BOT_NAME")

    # Notification setup
    OPENHOUSE_NOTIFIERS = _read_env("OPENHOUSE_NOTIFIERS")
    OPENHOUSE_TELEGRAM_BOT_TOKEN = _read_env("OPENHOUSE_TELEGRAM_BOT_TOKEN")
    OPENHOUSE_TELEGRAM_BOT_NOTIFY_WITH_IMAGES = \
        _read_env("OPENHOUSE_TELEGRAM_BOT_NOTIFY_WITH_IMAGES")
    OPENHOUSE_TELEGRAM_RECEIVER_IDS = _read_env(
        "OPENHOUSE_TELEGRAM_RECEIVER_IDS")
    OPENHOUSE_MATTERMOST_WEBHOOK_URL = _read_env(
        "OPENHOUSE_MATTERMOST_WEBHOOK_URL")
    OPENHOUSE_SLACK_WEBHOOK_URL = _read_env("OPENHOUSE_SLACK_WEBHOOK_URL")
    OPENHOUSE_APPRISE_NOTIFY_WITH_IMAGES = _read_env(
        "OPENHOUSE_APPRISE_NOTIFY_WITH_IMAGES")
    OPENHOUSE_APPRISE_IMAGE_LIMIT = _read_env(
        "OPENHOUSE_APPRISE_IMAGE_LIMIT")

    # Filters
    OPENHOUSE_FILTER_EXCLUDED_TITLES = _read_env(
        "OPENHOUSE_FILTER_EXCLUDED_TITLES")
    OPENHOUSE_FILTER_MIN_PRICE = _read_env("OPENHOUSE_FILTER_MIN_PRICE")
    OPENHOUSE_FILTER_MAX_PRICE = _read_env("OPENHOUSE_FILTER_MAX_PRICE")
    OPENHOUSE_FILTER_MIN_SIZE = _read_env("OPENHOUSE_FILTER_MIN_SIZE")
    OPENHOUSE_FILTER_MAX_SIZE = _read_env("OPENHOUSE_FILTER_MAX_SIZE")
    OPENHOUSE_FILTER_MIN_ROOMS = _read_env("OPENHOUSE_FILTER_MIN_ROOMS")
    OPENHOUSE_FILTER_MAX_ROOMS = _read_env("OPENHOUSE_FILTER_MAX_ROOMS")
    OPENHOUSE_FILTER_MAX_PRICE_PER_SQUARE = _read_env(
        "OPENHOUSE_FILTER_MAX_PRICE_PER_SQUARE")
...
{url}"""
    ...
    def loop_is_active(self):
        """Return true if OpenHouse should be crawling in a loop"""
    ...
    def has_website_config(self):
        """True if the OpenHouse website configuration is present"""
        return 'website' in self.config

    def website_session_key(self):
        """Secret session key for the OpenHouse website"""
        return self._read_yaml_path('website.session_key', None)

    def website_domain(self):
        """Domain that the OpenHouse website is hosted at"""
        return self._read_yaml_path('website.domain', None)

    def website_bot_name(self):
        """Name of the telegram bot used by the OpenHouse website to send messages"""
    ...
    def exclude_swaps(self):
        """Return true if swap apartments should be excluded"""
        return self._get_filter_config("exclude_swaps")

    def target_city(self):
        """Return the target city for filtering"""
        return self._read_yaml_path('target_city', None)

    def ai_provider(self):
        return self._read_yaml_path('ai_provider', None)

    def ai_api_key(self):
        return self._read_yaml_path('ai_api_key', None)

    def ai_model(self):
        return self._read_yaml_path('ai_model', None)
...
class CaptchaEnvironmentConfig(YamlConfig):
    ...
    def _get_imagetyperz_token(self):
        return Env.OPENHOUSE_IMAGETYPERZ_TOKEN() or super()._get_imagetyperz_token()  # pylint: disable=no-member

    def get_twocaptcha_key(self) -> str:
        """Return the currently configured 2captcha API key"""
        return Env.OPENHOUSE_2CAPTCHA_KEY() or super().get_twocaptcha_key()  # pylint: disable=no-member

    def get_capmonster_key(self) -> str:
        """Return the currently configured Capmonster API key"""
        return Env.OPENHOUSE_CAPMONSTER_KEY() or super().get_capmonster_key()

    def captcha_driver_arguments(self):
        """The list of driver arguments for Selenium / Webdriver"""
        if Env.OPENHOUSE_HEADLESS_BROWSER() is not None:
            return [
                "--no-sandbox",
                "--headless",
                "--disable-gpu",
...
class Config(CaptchaEnvironmentConfig):  # pylint: disable=too-many-public-methods
    """Class to represent OpenHouse configuration, built from a file, supporting
    environment variable overrides
    """

    def __init__(self, filename=None):
        if filename is None and Env.OPENHOUSE_TARGET_URLS() is None:
            raise ConfigException(
                "Config file loaction must be specified, or OPENHOUSE_TARGET_URLS must be set")
    ...
    def database_location(self):
        """Return the location of the database folder"""
        return Env.OPENHOUSE_DATABASE_LOCATION() or super().database_location()

    def target_urls(self):
        env_urls = Env.OPENHOUSE_TARGET_URLS()
        if env_urls is not None:
            return env_urls.split(';')
        return super().target_urls()

    def verbose_logging(self):
        if Env.OPENHOUSE_VERBOSE_LOG() is not None:
            return True
        return super().verbose_logging()

    def loop_is_active(self):
        if Env.OPENHOUSE_LOOP_PERIOD_SECONDS() is not None:
            return True
        return super().loop_is_active()

    def loop_period_seconds(self):
        env_seconds = Env.OPENHOUSE_LOOP_PERIOD_SECONDS()
        if env_seconds is not None:
            return int(env_seconds)
        return super().loop_period_seconds()

    def random_jitter_enabled(self):
        env_jitter = Env.OPENHOUSE_RANDOM_JITTER_ENABLED()
        if env_jitter is not None:
            return _to_bool(env_jitter)
        return _to_bool(super().random_jitter_enabled())

    def loop_pause_from(self):
        env_pause = Env.OPENHOUSE_LOOP_PAUSE_FROM()
        if env_pause is not None:
            return str(env_pause)
        return super().loop_pause_from()

    def loop_pause_till(self):
        env_until = Env.OPENHOUSE_LOOP_PAUSE_TILL()
        if env_until is not None:
            return str(env_until)
        return super().loop_pause_till()

    def has_website_config(self):
        if Env.OPENHOUSE_WEBSITE_SESSION_KEY() is not None:
            return True
        return super().has_website_config()

    def website_session_key(self):
        return Env.OPENHOUSE_WEBSITE_SESSION_KEY() or super().website_session_key()

    def website_domain(self):
        return Env.OPENHOUSE_WEBSITE_DOMAIN() or super().website_domain()

    def website_bot_name(self):
        return Env.OPENHOUSE_WEBSITE_BOT_NAME() or super().website_bot_name()

    def google_cloud_project_id(self):
        return Env.OPENHOUSE_GOOGLE_CLOUD_PROJECT_ID() or super().google_cloud_project_id()

    def message_format(self):
        env_message_format = Env.OPENHOUSE_MESSAGE_FORMAT()
        if env_message_format is not None:
            return '\n'.join(env_message_format.split('#CR#'))
        return super().message_format()

    def notifiers(self):
        env_notifiers = Env.OPENHOUSE_NOTIFIERS()
        if env_notifiers is not None:
            return env_notifiers.split(",")
        return super().notifiers()

    def telegram_bot_token(self) -> Optional[str]:
        return Env.OPENHOUSE_TELEGRAM_BOT_TOKEN() or super().telegram_bot_token()

    def telegram_notify_with_images(self) -> bool:
        env_bot_images = Env.OPENHOUSE_TELEGRAM_BOT_NOTIFY_WITH_IMAGES()
        if env_bot_images is not None:
            return str(env_bot_images) == 'true'
        return super().telegram_notify_with_images()

    def telegram_receiver_ids(self):
        env_receiver_ids = Env.OPENHOUSE_TELEGRAM_RECEIVER_IDS()
        if env_receiver_ids is not None:
            return [int(x) for x in env_receiver_ids.split(",")]
        return super().telegram_receiver_ids()

    def mattermost_webhook_url(self):
        return Env.OPENHOUSE_MATTERMOST_WEBHOOK_URL() or super().mattermost_webhook_url()

    def slack_webhook_url(self):
        if Env.OPENHOUSE_SLACK_WEBHOOK_URL() is not None:
            return Env.OPENHOUSE_SLACK_WEBHOOK_URL()
        return super().slack_webhook_url()

    def apprise_notify_with_images(self) -> bool:
        if Env.OPENHOUSE_APPRISE_NOTIFY_WITH_IMAGES() is not None:
            return str(Env.OPENHOUSE_APPRISE_NOTIFY_WITH_IMAGES()) == 'true'
        return super().apprise_notify_with_images()

    def apprise_image_limit(self) -> Optional[int]:
        env_limit = Env.OPENHOUSE_APPRISE_IMAGE_LIMIT()
        if env_limit is not None:
            return int(env_limit)
        return super().apprise_image_limit()

    def excluded_titles(self):
        env_filter = Env.OPENHOUSE_FILTER_EXCLUDED_TITLES()
        if env_filter is not None:
            return env_filter.split(";")
        return super().excluded_titles()

    def min_price(self):
        env_price = Env.OPENHOUSE_FILTER_MIN_PRICE()
        if env_price is not None:
            return int(env_price)
        return super().min_price()

    def max_price(self):
        env_price = Env.OPENHOUSE_FILTER_MAX_PRICE()
        if env_price is not None:
            return int(env_price)
        return super().max_price()

    def min_size(self):
        env_size = Env.OPENHOUSE_FILTER_MIN_SIZE()
        if env_size is not None:
            return int(env_size)
        return super().min_size()

    def max_size(self):
        env_size = Env.OPENHOUSE_FILTER_MAX_SIZE()
        if env_size is not None:
            return int(env_size)
        return super().max_size()

    def min_rooms(self):
        env_rooms = Env.OPENHOUSE_FILTER_MIN_ROOMS()
        if env_rooms is not None:
            return int(env_rooms)
        return super().min_rooms()

    def max_rooms(self):
        env_rooms = Env.OPENHOUSE_FILTER_MAX_ROOMS()
        if env_rooms is not None:
            return int(env_rooms)
        return super().max_rooms()

    def max_price_per_square(self):
        env_price = Env.OPENHOUSE_FILTER_MAX_PRICE_PER_SQUARE()
        if env_price is not None:
            return float(env_price)
        return super().max_price_per_square()

    def immoscout_cookie(self):
        return Env.OPENHOUSE_IS24_COOKIE() or super().immoscout_cookie()
