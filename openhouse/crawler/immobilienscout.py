...
from openhouse.schemas.immobilienscout import ImmoscoutQuery
...
class Immobilienscout(Crawler):
    ...
    def get_results(self, search_url: str, max_pages: int | None = None) -> list:
        ...
        page_no = 1
        listings_res = self.fetch_api_data(api_url, page_no)
        listings = listings_res.json() if listings_res.status_code == 200 else {}

        # If the API blocks us (412, missing totalResults, or Cloudfront WAF)
        if listings_res.status_code != 200 or "totalResults" not in listings:
            logger.info("ImmoScout API blocked us or requires WAF bypass. Launching Selenium...")
            try:
                from openhouse.chrome_wrapper import get_chrome_driver
                driver = get_chrome_driver(driver_arguments=None)
                try:
                    # Let the abstract crawler navigate to the URL and solve AWS WAF
                    self.get_page(search_url, driver=driver)
                    
                    # Extract AWS WAF and session cookies
                    cookie_header = ""
                ...
                # Retry the API call with the valid cookies
                logger.debug("Retrying Immoscout API with WAF bypass cookies...")
                listings_res = self.fetch_api_data(api_url, page_no)
                listings = listings_res.json() if listings_res.status_code == 200 else {}
            except ImportError:
                logger.warning("Selenium could not be loaded for WAF bypass.")
            except Exception as e:
                logger.warning(f"WAF bypass failed: {e}")

        no_of_results = listings.get("totalResults", 0)
