...
class Howoge(Crawler):
    """Implementation of Crawler interface for Howoge"""
    ...
    def extract_data(self, raw_data: BeautifulSoup):
        """Extracts all exposes from Howoge listing page"""
        entries = []
        listings = raw_data.find_all("div", class_=re.compile(r'flat-result|wohnungs|listing|immo'))
        if not listings:
            listings = raw_data.find_all("article")
        if not listings:
            listings = raw_data.find_all("a", href=re.compile(r'/wohnungen.*detail|/wohnung/'))
        ...
        logger.debug('Howoge: Number of entries found: %d', len(entries))
        return entries
