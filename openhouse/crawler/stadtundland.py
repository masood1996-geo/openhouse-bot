"""Expose crawler for Stadt und Land"""
import re
import hashlib
...
class StadtUndLand(Crawler):
    """Implementation of Crawler interface for Stadt und Land"""

    URL_PATTERN = re.compile(r'https://www\.stadtundland\.de')
    ...
    def extract_data(self, raw_data: BeautifulSoup):
        """Extracts all exposes from Stadt und Land listing page"""
        entries = []
        listings = raw_data.find_all("div", class_=re.compile(r'SP-TeaserList__item|immo|listing|result|angebot'))
        if not listings:
            listings = raw_data.find_all("article")
        if not listings:
            listings = raw_data.find_all("a", href=re.compile(r'/Wohnungssuche|/wohnung'))
        ...
        for listing in listings:
            ...
            if not url.startswith('http'):
                url = 'https://www.stadtundland.de' + url
            seen_urls.add(url)
            ...
            if img_tag:
                ...
                if image and not image.startswith('http'):
                    image = 'https://www.stadtundland.de' + image
        ...
        logger.debug('StadtUndLand: Number of entries found: %d', len(entries))
        return entries
